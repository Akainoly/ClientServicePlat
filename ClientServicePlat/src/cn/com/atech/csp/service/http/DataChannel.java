...
public class DataChannel {
	...
	public DataChannel(SocketChannel socketChannel, boolean blocking) throws IOException {
	...
	/*
	 * 如果原缓冲区的剩余容量不够，就创建一个新的缓冲区，容量为原来的两倍, 把原来缓冲区的数据拷贝到新缓冲区
	 */
	protected void resizeRequestBuffer(int remaining) {
		if (requestBuffer.remaining() < remaining) {
			// 把容量增大到原来的两倍
			ByteBuffer bb = ByteBuffer.allocate(requestBuffer.capacity() +remaining);
			requestBuffer.flip();
